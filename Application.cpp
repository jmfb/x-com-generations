...
int Application::Run(HINSTANCE instance, char* command, int show)
{
	...
	{
		ReportUnhandled();
		retval = static_cast<int>(Error::Codes::UnhandledException);
	}
	return retval;
}
...
int Application::RunMessageLoop()
{
	try
	{
		for (MSG msg;;)
		{
			...
			else if (!mPaused)
			{
				...
				UnitTest::Inject<IIdleMap>::Resolve()->OnIdle();
				OnPaint(false);
	...
	{
		ReportUnhandled();
		return static_cast<int>(Error::Codes::UnhandledException);
	}
}
...
void Application::OnPaint(bool validate)
{
	...
	UnitTest::Inject<IGame>::Resolve()->RenderScene();
